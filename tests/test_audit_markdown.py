...
class MarkdownAuditTests(unittest.TestCase):
    ...
    def test_linked_product_hero_is_not_counted_as_a_badge(self) -> None:
        anchored_english = EN_HOME.replace(
            '<img src="assets/hero.png" alt="Demo Board" width="500">',
            '<a href="https://example.com/product"><img src="assets/hero.png" alt="Demo Board" width="500"></a>',
        )
        anchored_chinese = ZH_HOME.replace(
            '<img src="assets/hero.png" alt="Demo Board" width="500">',
            '<a href="https://example.com/product"><img src="assets/hero.png" alt="Demo Board" width="500"></a>',
        )
        self.write("README.md", anchored_english)
        self.write("README_ZH.md", anchored_chinese)
        config = audit_markdown.load_config(None)
        config["homepage_pairs"][0]["profile"] = "single-product"
        report = self.report(self.changes("README.md", "README_ZH.md", status="M"), config=config)
        missing = [
            item for item in report["findings"]
            if item["code"] == "HOMEPAGE_REQUIRED_COMPONENT_MISSING"
            and "hero_image" in item["message"]
        ]
