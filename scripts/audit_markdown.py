    ...
    for href, body in HTML_ANCHOR_RE.findall(header):
        ...
        for key, pattern in BADGE_RULES:
            if pattern.search(searchable):
                role = key
                break
        # A product hero is often wrapped in a link just like a badge.  Only
        # count images with an actual badge role so the remaining image can
        # satisfy the single-product hero contract.
        if role is not None:
            result.append(role)
    return result
