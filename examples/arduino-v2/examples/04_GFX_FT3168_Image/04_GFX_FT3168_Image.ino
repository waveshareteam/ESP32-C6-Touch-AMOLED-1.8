...
void setup() {
  ...
  USBSerial.printf("ID: %#X \n\n", (int32_t)CST820->IIC_Read_Device_ID());
  delay(1000);
}

void loop() {
  if (CST820->IIC_Interrupt_Flag == true) {
    CST820->IIC_Interrupt_Flag = false;

    int32_t touch_x = CST820->IIC_Read_Device_Value(CST820->Arduino_IIC_Touch::Value_Information::TOUCH_COORDINATE_X);
    int32_t touch_y = CST820->IIC_Read_Device_Value(CST820->Arduino_IIC_Touch::Value_Information::TOUCH_COORDINATE_Y);
    uint8_t fingers_number = CST820->IIC_Read_Device_Value(CST820->Arduino_IIC_Touch::Value_Information::TOUCH_FINGER_NUMBER);
